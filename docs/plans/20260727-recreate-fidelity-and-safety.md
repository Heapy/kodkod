...
- [x] тест-на-баг: rollback на потоке с установленным флагом прерывания обязан отработать (сейчас NIO падает
      мгновенно и обе операции проглатываются)
- [x] в `rollback()` снимать флаг прерывания (`Thread.interrupted()`) перед вызовами и восстанавливать после
- [x] в shutdown-хуке: `scheduler.shutdown()` + `awaitTermination(KODKOD_SHUTDOWN_GRACE, default 30s)`, затем
      `shutdownNow()`; добавить переменную в `Config`
- [x] реализовать reconcile: поиск по `all=true` контейнеров с именем `<name>_kodkod_old_<12 hex собственного id>`
      ...
      (листинг сужен фильтром `name=_kodkod_old_`, иначе в фикстуры попадают все контейнеры хоста)
- [x] тесты: осиротевший бэкап без замены → переименован и запущен; бэкап при работающей замене → удалён;
...
- [x] e2e: убить kodkod между `rename` и `start` (на базе `compose.rollback.yml`), перезапустить, убедиться что
      сервис вернулся (состояние окна воспроизведено через CLI — `kill` kodkod, затем `stop` + `rename` контейнера
      в бэкап-имя: попадание в реальное окно между `rename` и `start` недетерминировано и дало бы flaky-тест)
- [x] перезаписать фикстуры (в начале цикла появился `all=true` листинг)
- [x] прогнать тесты и целевой e2e; снять пункт 1 из `TODO.md`
...
- [x] тест-на-баг: два цикла подряд с неподнимающимся образом → второй цикл не должен ни останавливать, ни
      переименовывать контейнер (фейковые часы из задачи 3) —
      `an_image_that_could_not_come_up_is_not_tried_again_next_cycle`: во втором цикле в `ops` остаётся
      ровно `pull:`, контейнер по-прежнему запущен
- [x] добавить память известных плохих пар `(containerId, imageId) -> времяПоследнейПопытки`; запись сбрасывается,
      когда у тега появляется другой image id, и при успешном recreate (`Updater.failedUpdates`,
      `rememberFailedUpdate`/`suppressedByCooldown`; пишется только для `stale`-цели — у recreate по
      зависимости нет нового образа, и подавлять его значило бы оставить контейнер с мёртвым netns)
- [x] добавить `KODKOD_UPDATE_FAILURE_COOLDOWN` (default 21600 = 6ч) — один плоский cooldown, без экспоненты
      (`0` — задокументированный выключатель памяти)
- [x] логировать пропуск явно («образ sha256:… уже провалился, следующая попытка не раньше …») — плюс строка в
      момент записи: провал уже громкий, и оператор узнаёт срок следующей попытки сразу, а не через час
- [x] тесты: cooldown истёк → попытка повторяется; вышел новый image id → память сброшена, попытка сразу;
      успешный recreate очищает запись (`an_update_that_finally_worked_is_not_remembered_as_a_failure`;
      ⚠️ строчка `failedUpdates.remove` на успешном пути принципиально ненаблюдаема через `runOnce`: успешный
      recreate меняет id контейнера, а ключ памяти — id, так что запись мертва в любом случае. Тест
      утверждает саму гарантию — «контейнер, обновление которого прошло, не уносит cooldown в следующие
      циклы» — и краснеет на реалистичной ошибке «писать запись на попытку, а не на провал»)
- [x] прогнать тесты (`./gradlew test` 156/156 и `compileE2eTestKotlin` — зелёные; корпус фикстур не менялся:
      задача не трогает ни один путь запроса, а записанные сценарии — один цикл без предыстории.
      Краснота подтверждена двумя нейтрализациями: без `rememberFailedUpdate` падает ровно тест-на-баг,
      без сброса записи падают все три теста на сброс)
...
- [x] тест-на-баг: контейнер, остающийся unhealthy, не должен рестартоваться каждые 30 секунд бесконечно
      (фейковые часы из задачи 3) (`does_not_restart_a_still_unhealthy_container_every_interval` — красный на
      текущем коде: рестарт в каждом из 5 циклов вместо трёх)
- [x] добавить экспоненциальный backoff на контейнер с потолком `KODKOD_AUTOHEAL_MAX_INTERVAL` (default 3600)
      (`Autoheal.restarts`: счётчик попыток + время последней; окно `interval * 2^(n-1)`, считается удвоением,
      а не сдвигом, чтобы большой потолок не переполнил экспоненту. Потолок в `Config` поднимается до
      `KODKOD_AUTOHEAL_INTERVAL`, если задан ниже — иначе он ничего не ограничивает; равенство потолка и
      интервала выключает backoff. Попытка засчитывается **до** вызова: рестарт, который демон отверг, — тоже
      не повод спрашивать каждый цикл)
- [x] сбрасывать счётчик, когда контейнер вернулся в healthy или исчез (выздоровевший/удалённый/выпавший из
      мониторимого набора контейнер просто отсутствует в листинге `health=unhealthy` — сброс по отсутствию
      закрывает все три случая одним `retainAll`)
- [x] тесты: интервал растёт 30 → 60 → 120 …; упирается в потолок; сбрасывается после выздоровления; разные
      контейнеры считаются независимо; конфиг-тест на новую переменную (`the_wait_between_restarts_doubles`,
      `..._stops_growing_at_the_ceiling`, `the_backoff_resets_once_the_container_is_healthy_again`,
      `counts_the_restarts_of_each_container_separately`, `reads_the_autoheal_backoff_ceiling`)
- [x] прогнать тесты; снять пункт 7 из `TODO.md` (`./gradlew test` 180/180 и `compileE2eTestKotlin` — зелёные,
      фикстуры не перезаписывались: путь запросов не изменился, `DockerReplayTest` по-прежнему
      `isFullyConsumed()`. В `TODO.md` снят пункт про backoff — после двух переномераций это был пункт 5, —
...
- [x] тест-на-баг: немаркированный sidecar с `network_mode: container:app` при обновлении `app` обязан быть
      пересоздан/перезапущен, а не остаться с мёртвым netns (требует фильтр-aware листинг из задачи 3)
      (`an_unlabelled_netns_sidecar_is_recreated_when_its_provider_is_replaced`; в `UpdaterTest` листинг
      теперь несёт `Names`/`State`/`HostConfig`/`NetworkSettings`, как настоящий `/containers/json` —
      без них create-time зависимость вообще не опознаётся из листинга)
- [x] использовать `Dependents.kt` из задачи 15 в `Updater`: искать create-time зависимых по всему демону
      (`Updater.refreshCreateTimeDependents`, один проход после всего bring-back'а; ширина скана — та же
      дисциплина, что в задаче 15: сначала листинг, сужённый `label=com.docker.compose.project`, и только
      проект с реальным шерингом дочитывается нефильтрованным `all=true`)
- [x] пересоздавать/перезапускать найденных после замены провайдера; если пересоздать нельзя — явный WARN
      вместо тишины (выбор по тому, **как записана ссылка** — новое поле `Dependent.pinnedToProviderId`:
      ссылка на id провайдера умирает вместе с контейнером, `restart` получил бы `No such container` и
      оставил бы sidecar ещё и остановленным, поэтому такой зависимый пересоздаётся против имени замены;
      ссылка по имени и провайдер, который цикл только перезапустил, лечатся `restart`'ом. WARN на всех
      неудачах: несделанный inspect, упавший recreate, а также на остановленном зависимом — его мы не
      запускаем, но молчать о create-time ссылке в никуда нельзя)
- [x] тесты: sidecar внутри мониторимого набора (как раньше), sidecar снаружи, legacy `--link` снаружи
      (`a_monitored_netns_sidecar_is_still_recreated_exactly_once` — негативный контроль на двойное
      пересоздание: замена, сделанная графом, ссылается на провайдера уже **по имени** и без исключения
      по `liveId` была бы найдена как «новый» зависимый; плюс
      `a_stopped_sidecar_outside_the_monitored_set_is_reported_rather_than_started` и
      `a_sidecar_that_cannot_be_recreated_is_rolled_back_and_reported` — ошибочный путь)
- [x] e2e: расширить `compose.container-mode.yml` немаркированным потребителем, проверить наличие сети после
      обновления провайдера (сервис `sidecar` без kodkod-лейблов; ассерт не на факт пересоздания, а на
      `eth0` внутри контейнера — контейнер с убитым namespace по-прежнему `Running` и с правдоподобным
      `NetworkMode`, и разница видна только изнутри: помощник `E2eHarness.interfaces`)
- [x] ➕ перезаписать фикстуры (в конце цикла появился сужённый листинг проекта): перезаписаны только
      `update-recreate` и `deps-ordered` в том же лейбле `engine-29.6.2_compose-5.3.1` — `update-noop`
      (нечего перезапускать) и `autoheal-restart` новый путь не трогают. Нормализованная сверка манифестов:
      18→19 и 24→26 обменов, единственная структурная разница — 1 и 2 новых
      `GET /containers/json?all=true&filters={"label":["com.docker.compose.project=…"]}`; в их телах только
      контейнеры сценария, нефильтрованного `all=true` в корпусе не появилось
- [x] ➕ prune старого образа привязан к `target.stale` (не в исходном объёме): recreate по зависимости
      оставляет замену на **том же** образе, так что `DELETE /images/<old>` — это просьба удалить живой
      образ (демон отвечает 409, в лог уходит шум), и для немаркированного sidecar'а это был бы ещё и
      запрос на удаление образа контейнера, которым kodkod не управляет
- [x] прогнать тесты и целевой e2e (`./gradlew test` 199/199, `compileE2eTestKotlin` — зелёные;
      `./gradlew e2eTest --tests '*containerNetworkMode*'` на DinD — зелёный. Краснота подтверждена
      нейтрализацией вызова `refreshCreateTimeDependents`: падают 4 из 5 новых тестов, пятый — негативный
      контроль на двойное пересоздание, он и должен оставаться зелёным)
...
- [x] тест-на-баг: фаза планирования не должна выполнять мутирующих операций — ассерт «в `ops` после `plan()` нет
      ничего, кроме `pull:`» (сам pull остаётся в фазе чтения и пишется в `ops` обеими обвязками)
      (`planning_pulls_but_changes_nothing`: ассерт `ops == ["pull:nginx:1.27"]` при stale-контейнере и
      осиротевшем бэкапе в наборе — то есть и pull состоялся, и ни одна мутация, включая reconcile, не ушла
      до взятия лока)
- [x] разделить `runOnce()` на `plan(): UpdatePlan` (только чтение) и `apply(plan)` (мутации)
      (`runOnce() = apply(plan())` остался публичным — им пользуются e2e-рекордер и replay-тесты; `plan`/`apply`
      и `UpdatePlan` — `internal`, так как `Target` internal. `UpdatePlan` несёт весь монитори́руемый набор в
      топологическом порядке плюс производный `work`/`hasWork`)
- [x] в `Main` брать `cycleLock` только вокруг `apply`; в комментарии зафиксировать, почему pull больше не держит
      лок и каким верхним пределом ограничены ожидания внутри `apply` (liveness-гейт из задачи 10 и
      `service_healthy` из задачи 17) (новый `updateCycle`: `plan()` без лока, `guarded("update", cycleLock)`
      только вокруг `apply`; в KDoc — 600s idle на pull как причина и перечень ограниченных ожиданий внутри
      `apply`: стопы, `KODKOD_UPDATE_VERIFY_SECONDS`, `KODKOD_DEPENDENCY_HEALTH_TIMEOUT`)
- [x] в начале `apply` перепроверять актуальность плана (контейнер существует, image id не изменился) — состояние
      могло измениться, пока качался образ (`isCurrent`: `inspectContainer` по каждому `work`-таргету, сверка
      `Image` с `Target.currentImageId`; план отбрасывается **целиком** — таргеты это граф зависимостей, и
      применить «уцелевшую половину» значит остановить контейнеры ради зависимости, которую уже не обновляют)
- [x] тесты: план на устаревшем состоянии отбрасывается с логом; обычный путь не изменился
      (`a_plan_whose_container_disappeared_is_dropped`, `a_plan_whose_image_moved_under_it_is_dropped` — обе
      проверяют и отсутствие мутаций, и лог «dropping this cycle's plan»;
      `applying_the_plan_is_what_recreates_the_container` — обычный путь через явные `plan()`+`apply()`;
      остальные 199 тестов по-прежнему идут через `runOnce()` и не менялись)
- [x] перезаписать фикстуры, если порядок или состав запросов изменился; иначе явно убедиться, что не изменился
      (перезаписаны только `update-recreate` и `deps-ordered` в том же лейбле `engine-29.6.2_compose-5.3.1`;
      `update-noop` и `autoheal-restart` не тронуты — там нечего мутировать, а значит и перепроверять.
      Нормализованная сверка манифестов: 19→20 и 26→28 обменов, ровно две структурные разницы — листинг
      reconcile (`GET /containers/json?all=true&filters={"name":["_kodkod_old_"]}`) переехал с первого места
      в начало мутирующей фазы, и добавились 1 и 2 `GET /containers/<id>/json` от `isCurrent`)
- [x] прогнать тесты (`./gradlew test` 204/204 и `compileE2eTestKotlin` — зелёные. Краснота подтверждена
      нейтрализацией обоих изменений: `isCurrent` → `return true` и `reconcileOrphanedBackups()` обратно в
      `plan()` — падают все 5 новых тестов плюс `an_orphaned_backup_with_nothing_serving_the_name_is_restored`
      на удвоенном reconcile)
- [x] ➕ reconcile осиротевших бэкапов (задача 13) отнесён к `apply`: он переименовывает и запускает контейнеры,
      то есть должен быть под тем же локом, что и остальные мутации. Остался первым в цикле и безусловным —
      `apply` вызывается каждый цикл, даже с пустым планом, а в `Main` неудача планирования деградирует до
      `UpdatePlan.NOTHING` вместо пропуска мутирующей половины: упавший листинг не повод оставить сервис
      припаркованным под бэкап-именем ещё на интервал (`a_dropped_plan_does_not_take_the_reconcile_with_it`).
      Цена — восстановление сироты теперь после фазы чтения, а не до неё; сироту внутри живого процесса
      создаёт только SIGKILL (он же процесс и завершает), а стартовый вызов из `main` не изменился
...
- [x] привести README в соответствие с фактическим поведением: строки про сохранение `HostConfig/volumes/labels`
      (91), про rollback (92-93) и про удаление старого образа (73); добавить новые `KODKOD_*` в таблицу —
      «What carries over to the replacement» перечисляет по факту (HostConfig с двумя правками, анонимные
      тома по имени, лейблы минус дефолты образа и с перештампованным `com.docker.compose.image`, endpoint'ы
      с `IPAMConfig`/`GwPriority`, platform); rollback описан как «снять замену → освободить имя → вернуть
      имя → запустить → проверить инспектом», с `ROLLBACK INCOMPLETE` и reconcile'ом бэкапа после SIGKILL;
      строка `KODKOD_UPDATE_CLEANUP` — про пропуск образа с чужим тегом. В таблицу добавлены все 7 новых
      переменных (сверено с `Config.fromEnv`: 20 переменных, все в таблице), `KODKOD_STOP_TIMEOUT` уточнена
      («no `t` is sent at all»)
- [x] задокументировать решение по `depends_on.restart` и переменную `KODKOD_RESPECT_DEPENDS_ON_RESTART` в разделе
      «Ordering & dependencies» (подраздел «`depends_on.restart` is opt-in»: `condition` соблюдается всегда,
      `restart` — только под флагом, потому что `restart: false` — дефолт спеки; и даже под флагом он не может
      подавить recreate create-time зависимого)
- [x] обновить `CHANGELOG.md` — сюда же попадают незаписанные пункты 9 из `TODO.md` (decoupling за `DockerClient`,
      JUnit-миграция e2e, CI job) и всё из этого плана (плюс NUL-fix в service key)
- [x] почистить `TODO.md`: снять закрытые пункты (1, 5, 7, частично 10), оставшиеся — переформулировать без
      протухших ссылок на строки; добавить отложенный хвост из Post-Completion, включая `wise-mapping-shell.md` —
      файл не удалён и не оставлен в TODO, а перенесён в `docs/design-record-replay-docker-fixtures.md`
      (260-строчный дизайн-документ этой самой обвязки под несвязанным именем). TODO переструктурирован в
      4 раздела (18 пунктов), добавлены два частично закрытых места (MAC на Docker ≥27, health-ветка гейта)
      и находка задачи 20 (grace-хук в `Main.kt` без теста)
- [x] описать рекордер фикстур в `E2E_TESTING.md`, включая обязательные флаги и то, что он пишет в корпус
      (раздел «Recording Docker fixtures»: оба флага, отказ при чужом демоне, пути записи + `index.json`,
      правило «изменил метод/путь/query — перезаписал корпус», почему тела не в ключе, `VERIFY_HEALTH=false`;
      в матрицу добавлены два новых сценария и `FixtureWriterTest`)
- [x] обновить `CLAUDE.md`, если появились новые паттерны (правило «ops пишутся после успешного вызова»,
      правило «изменил путь запроса — перезаписал фикстуры») — файла не существовало, создан: архитектурные
      правила (socket-only, `DockerClient`, env-конфиг, инъекция времени, plan/apply), тест-дубли (`!`-маркер),
      корпус фикстур (перезапись, запрет неотфильтрованного `all=true` на пути рекордера)
- [ ] перенести этот план в `docs/plans/completed/` — **статус: не выполнено и вручную не выполняется**;
      перенос делает харнесс после review/finalize-фаз, поэтому пункт остаётся снятым, пока файл лежит здесь

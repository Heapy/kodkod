...
class KodkodE2eTest {
    ...
    fun multiNetworkContainerIsReconnectedToEveryNetwork() = e2e.scenario("multinet") {
        ...
        val explicitMac = configMac("e2e-multinet-mac-1")
        val macBefore = endpointMac("e2e-multinet-mac-1", "e2e-multinet_neta")
        ...
        // The `mac` service asked for a MAC explicitly. kodkod may only carry it over when the engine
        // still exposes `Config.MacAddress` — that field is what separates a requested MAC from the one
        // the daemon generates for every endpoint, and Docker >= 27 no longer reports it at all.
        waitUntil(90, "mac service updated to v2") { variant("e2e-multinet-mac-1") == "v2" }
        val macAfter = endpointMac("e2e-multinet-mac-1", "e2e-multinet_neta")
        if (explicitMac.isNotEmpty()) {
            assertEquals(macBefore, macAfter, "an explicitly configured MAC must survive the update")
        } else {
            assertTrue(
                macAfter.isNotEmpty(),
                "engine ${engineVersion()} hides Config.MacAddress, so the requested MAC cannot be told from a " +
                    "generated one and is deliberately not pinned; the replacement must still have an endpoint MAC",
            )
        }
    }

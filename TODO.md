...
5. MacAddress теряется в cleanEndpoint при recreate (моя рек. #5b).
6. Один глобальный KODKOD_REGISTRY_AUTH vs per-registry auth у watchtower.
7. Backoff для «хлопающего» unhealthy — нет (общий грех с обоими docker-autoheal).
8. Версии разъехались: build.gradle.kts = 1.0.0, а теги v0.2.0/v0.3.0 и CHANGELOG сверху [Unreleased]. Решить: резать 1.0.0 или вернуть версию к реальности.
9. CHANGELOG отстал: decoupling за DockerClient, JUnit-миграция e2e, CI-job, NUL-fix — не записаны в [Unreleased].
10. --link все еще заявлен как supported create-time dependency, но при recreate HostConfig.Links не переписывается. resolveLinks видит legacy links, но recreate передает HostConfig почти как есть, только меняет NetworkMode. У watchtower есть отдельный GetCreateHostConfig, который нормализует links перед create. См. Updater.kt (line 217), Updater.kt (line 407), reference container.go (line 350).
11. pinned image by image id sha256:... не пропускается. Сейчас skip только для image@sha256:...; если Config.Image будет sha256:<id>, код пойдет в splitImageRef, затем в registry/pull path как будто это repo:tag. Watchtower явно отсекает strings.HasPrefix(imageName, "sha256:"). См. Updater.kt (line 124), Updater.kt (line 471), reference client.go (line 361).